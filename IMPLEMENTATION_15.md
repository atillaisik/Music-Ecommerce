...
- [x] **Error Propagation**: Update `useWishlistStore` in `src/lib/store.ts` to `throw` errors from `wishlistAPI` so components can catch them.
- [x] **Robust Syncing**: Enhance `syncWishlist` to handle failures on individual items (e.g., if a product doesn't exist in the DB) without failing the whole sync.
